...
class CliGoUtTestsGenerationController(
    ...
    override fun onSourceCodeAnalysisStart(targetFunctionsNamesBySourceFiles: Map<String, List<String>>): Boolean {
        ...
        cliLogger.debug { "Source code analysis - started" }
    ...
    override fun onSourceCodeAnalysisFinished(
        ...
        cliLogger.debug { "Source code analysis - completed in [$stageDuration] (ms)" }
    ...
    override fun onTestCasesGenerationForGoSourceFileFunctionsStart(
        ...
        currentStageStarted = now()
        cliLogger.debug { "Generating test for [${sourceFile.fileName}] - started" }
    ...
    override fun onTestCasesFileCodeGenerationStart(
        ...
        currentStageStarted = now()
        cliLogger.debug { "Test cases file code generation for [${sourceFile.fileName}] - started" }
    ...
    private fun writeGeneratedCodeToFile(sourceFile: GoUtFile, generatedTestsFileCode: String) {
        ...
        if (testFile.exists()) {
            ...
            if (overwriteTestFiles) {
                cliLogger.warn { "$alreadyExistsMessage: it will be overwritten" }
            } else {
                cliLogger.warn { "$alreadyExistsMessage: skipping test generation for [${sourceFile.fileName}]" }
                return
            }
        }
