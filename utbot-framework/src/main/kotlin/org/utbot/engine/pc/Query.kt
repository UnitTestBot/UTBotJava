...
data class Query(
    ...
    private fun findNotAddedConstraints(
        ...
        newHardHashCode.forEach { (hash, expr) ->
            ...
            // In case of a collision, we should add expr if an element in hardHashCode is not equal to it.
            // If we have multiple collisions (i.e., several distinct elements from hard have the same hash)
            // we will put in the result more constraints that it is required to,
            // but we won't lose none, and we'll increase performance, so, that's ok.
            if (expr != hardHashCode[hash]) {
                result += expr
            }
        }
