...
object TaintEntityParser {
     ...
     */
    fun parseTaintEntities(node: YamlNode): YamlTaintEntities =
        when (node) {
            is YamlScalar -> {
                YamlTaintEntitiesSet(setOf(taintEntityByName(node.content)))
            }
            ...
            is YamlList -> {
                ...
                YamlTaintEntitiesSet(entities.toSet())
            }
    ...
    /**
     * Constructs [YamlTaintEntity] by the given [name] &ndash "this", "return" or "argN".
     */
    fun taintEntityByName(name: String): YamlTaintEntity =
        when (name) {
            Constants.KEY_THIS -> YamlTaintEntityThis
            Constants.KEY_RETURN -> YamlTaintEntityReturn
            else -> {
