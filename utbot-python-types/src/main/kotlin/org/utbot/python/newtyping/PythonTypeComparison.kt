...
class PythonTypeWrapperForEqualityCheck(
    ...
    private fun equalChildren(
        selfChildren: List<UtType>,
        otherChildren: List<UtType>,
    ...
    private fun getChildWrapper(elem: UtType): PythonTypeWrapperForEqualityCheck {
...
fun typesAreEqual(left: UtType, right: UtType): Boolean {
...
class PythonSubtypeChecker(
    ...
    fun rightIsSubtypeOfLeft(): Boolean {
        ...
        if (rightMeta is PythonTypeAliasDescription)
            return PythonSubtypeChecker(
                left = left,
                right = PythonTypeAliasDescription.getInterior(right),
                pythonTypeStorage,
    ...
    private fun reverse(correspondence: List<Pair<UtType, UtType>>): List<Pair<UtType, UtType>> =
    ...
    companion object {
        fun checkIfRightIsSubtypeOfLeft(left: UtType, right: UtType, pythonTypeStorage: PythonTypeStorage): Boolean =
            PythonSubtypeChecker(
                left = left,
                right = right,
                pythonTypeStorage,
