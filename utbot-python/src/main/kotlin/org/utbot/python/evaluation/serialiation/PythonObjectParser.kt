        ...
        val obj = when (this) {
            ...
            is ReduceMemoryObject -> {
                ...
                when (stateObjs) {
                    ...
                    is ListMemoryObject -> {
                        val state = stateObjs.toPythonTree(memoryDump, visited)
                        require(state is PythonTree.TupleNode) { "Field `setstate` must be tuple" }
                        prevObj.setstate = state
                    }
                    else -> {}
                }
